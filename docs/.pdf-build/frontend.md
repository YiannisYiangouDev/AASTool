...
└── lib/                             # Shared utilities
    ...
    ├── queryClient.ts               # React Query client configuration│   ├── theme.ts                     # Color/gradient tokens + gauge helpers
│   ├── storage.ts                   # localStorage persistence helpers    ├── animations.ts                # Framer Motion animation presets
...
|------|------|-------------|
| `building` | `string` | Building type name (fetched from API — dynamic, not hardcoded) |
...
**Response Unwrapping**: The `unwrap<T>()` helper extracts `response.data.data` (or falls back to `response.data`). The `evaluate` endpoint is a special case — it extracts `res.data.result` directly, since the evaluate API returns `{ ok: true, result: {...} }` rather than `{ ok: true, data: {...} }`.
...
│     - Saves certification scores, dashboard state       │
│     - Managed via lib/storage.ts helpers                │
│     - Keys: STORAGE_KEY, CERTIFICATION_KEY              │
└─────────────────────────────────────────────────────────┘
```
...
Centralized color and gradient tokens used across the app:

```ts
// Disability Type colors (DT1–DT5)
export const DT_COLORS: Record<string, string> = { ... };
// Assessment Dimension colors (AD1–AD5)
export const AD_COLORS: Record<string, string> = { ... };
// Dashboard gradient tokens
export const DT_GRADIENT: string = '...';
export const AD_GRADIENT: string = '...';
// OBS gauge dynamic colors
export function getObsGaugeColors(obs: number): { color1: string; color2: string };
// NEB badge gradient per class
