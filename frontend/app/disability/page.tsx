...
export default function DisabilityPage() {
  const { data: disabilityTypes = [], isLoading, error } = useQuery({
    queryKey: ['disability-types'],
    queryFn: () => endpoints.getDisabilityTypes(),
  });
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-white selection:bg-teal-500/30">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-0 left-1/4 w-[600px] h-[600px] bg-teal-500/5 rounded-full blur-[120px]" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-amber-500/[0.03] rounded-full blur-[150px]" />
      </div>
      ...
      <main id="main-content" className="relative z-10 px-4 sm:px-6 lg:px-8 py-16 max-w-7xl mx-auto pt-28 pb-28">
        ...
        <div className="text-center mb-20">
          ...
          <h1 className="text-5xl sm:text-6xl lg:text-7xl font-black tracking-tight leading-[1.05]">
            Disability <span className="bg-gradient-to-r from-teal-400 via-violet-400 to-rose-400 bg-clip-text text-transparent">Types</span>
          </h1>
          <p className="text-slate-400 mt-5 text-lg max-w-2xl mx-auto leading-relaxed">
            Five core disability categories that define user persona groups. Each type drives a dedicated subset of assessment criteria.
          </p>
          <div className="mt-6 flex items-center justify-center gap-6 text-sm text-slate-500">
            <span className="flex items-center gap-1.5">
              <span className="w-8 h-[1px] bg-teal-500/40" />
              {disabilityTypes.length} categories
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-8 h-[1px] bg-violet-500/40" />
        ...
        {error && (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold text-white mb-2">Failed to load disability types</h2>
            ...
            {disabilityTypes.map((disabilityType: DisabilityType, idx: number) => {
              ...
              return (
                <div
                  ...
                  className={`group relative p-6 sm:p-8 rounded-2xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm hover:bg-white/[0.05] transition-all duration-500 hover:-translate-y-1 hover:border-white/[0.12] hover:shadow-2xl`}
                >
                  {/* Number ribbon */}
        ...
        {!isLoading && !error && (
          <div className="mt-20 text-center">
            <div className="inline-flex flex-col items-center gap-4 p-8 sm:p-12 rounded-3xl border border-white/[0.06] bg-white/[0.02] backdrop-blur-sm max-w-2xl w-full">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-violet-500/20 to-teal-500/20 ring-1 ring-violet-500/30">
              ...
              <h2 className="text-2xl font-bold text-white">Explore the criteria</h2>
              <p className="text-slate-400 text-sm max-w-md">
                See how each disability type maps to specific evaluation criteria across all five assessment dimensions.
              </p>
              <Link
                href="/criteria"
                className="inline-flex items-center gap-3 px-8 py-3.5 rounded-full bg-gradient-to-r from-violet-500 to-teal-500 text-white font-semibold text-sm hover:from-violet-400 hover:to-teal-400 hover:shadow-lg hover:shadow-violet-500/25 transition-all duration-300 hover:scale-[1.03] active:scale-95"
              >
                View Criteria
