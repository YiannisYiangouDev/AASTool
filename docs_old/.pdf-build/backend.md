...
- Generates human-readable titles: `"Accessibility Report #N — {building_type} ({date}) — OBS: {obs}%"`

### 6. Auth Controller (`authController.ts`)

**`POST /api/v1/login`** — Development-only mock authentication.
...
**Note**: This is a placeholder — replace with real JWT/OAuth before production deployment.
